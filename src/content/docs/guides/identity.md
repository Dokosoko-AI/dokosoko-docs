...
Open **Identity → Customer sign-in** and configure:
...
![The Customer sign-in screen shows the active OIDC revision and its fixed access-evaluation configuration.](/screenshots/customer-sign-in.jpg)
...
Open **Identity → Customer accounts** to review those durable accounts and suspend a compromised customer without changing the shared OIDC connection.

![The Customer accounts screen provides a separate suspension control for each resolved customer.](/screenshots/customer-accounts.jpg)
...
The delegated vendor token is used only for the fixed access-evaluation request. The resulting DokoSoko token is never forwarded to vendor services. See [Define authorization policies](/guides/authorization-policies/) for the separate grant, action, and confirmation rules attached to tools.
